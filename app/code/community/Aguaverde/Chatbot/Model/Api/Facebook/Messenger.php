<?php
	 ...
	 */
// class Aguaverde_Chatbot_Model_Api_Facebook_Messenger
	class Messenger {

		private $bot_id = "EAACuZAqIElcoBABKsBBoaRhZBxkelGJr6yZAUkanpYKZCkm0lbC6EOeygvgZCGF7AMVZBXsKZBW1VV41koqIjtG0yZBQm6GZAQaLbMVp4wpRydkkMXKFVcmKTOm9FjC7K5STljfZApVoWVt9ZAZBRfhheQNFb4WdJC9ZAV0XS4qjN5yijdAZDZD";
		...
		public function verifyWebhook($hub_token) {
     		Mage::log("verifyWebhook: hub_token \n" . var_export($hub_token, true) . "\n\n", null, 'chatbot_facebook.log');

            $req_hub_token = Mage::app()->getRequest()->getParam('hub_verify_token');

     		Mage::log("verifyWebhook: data hub_verify_token \n" . var_export($req_hub_token, true) . "\n\n", null, 'chatbot_facebook.log');


			if ($req_hub_token === $hub_token) {
			  $req_hub_challenge = Mage::app()->getRequest()->getParam('hub_challenge');	
			  return $req_hub_challenge;
			}

/*
			if ($this->data['hub_verify_token'] == $hub_token) {
      		    
      		    Mage::log("verifyWebhook: hub_challenge \n" . var_export($this->data['hub_challenge'], true) . "\n\n", null, 'chatbot_facebook.log');
		...
		public function endpoint($api, array $content, $post = true) {
			...
			$url = 'https://graph.facebook.com/v2.6/me/messages?access_token=EAACuZAqIElcoBABKsBBoaRhZBxkelGJr6yZAUkanpYKZCkm0lbC6EOeygvgZCGF7AMVZBXsKZBW1VV41koqIjtG0yZBQm6GZAQaLbMVp4wpRydkkMXKFVcmKTOm9FjC7K5STljfZApVoWVt9ZAZBRfhheQNFb4WdJC9ZAV0XS4qjN5yijdAZDZD';
			if ($post)
		...
		public function getData() {
			if (!($this->data)) {
				...
				#return json_decode($rawData, true);
	     		Mage::log("get data:\n" . var_export($rawData, true) . "\n\n", null, 'chatbot_facebook_rawdata.log');
